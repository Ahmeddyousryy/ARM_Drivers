...
void RCC_Void_InitClockSystem(void)
{
  ...
  //  while(GET_BIT(RCC_CR,1)==0)!;         // Wait until HSI to be ready
  ...
  //  while(GET_BIT(RCC_CR,17)==0);  	    // Wait until HSE to be ready
  ...
  //  while(GET_BIT(RCC_CR,25)==0);  	    // Wait until HSE to be ready
